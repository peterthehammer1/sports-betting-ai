...
export function SuperBowlAIAnalysis() {
  ...
  const ConfidenceBadge = ({ confidence }: { confidence: number }) => {
    const color = confidence >= 75 ? 'text-emerald-400 bg-emerald-500/10' :
  ...
  if (loading) {
    return (
      <div className="p-12 text-center">
        <div className="inline-flex items-center gap-3 px-6 py-4 bg-slate-900/60 rounded-xl border border-slate-800/50">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 bg-gradient-to-r from-slate-900 via-slate-900/80 to-slate-900 rounded-xl border border-slate-800/50">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-sm font-semibold text-slate-400 uppercase tracking-wider">Predicted Final Score</h3>
          <ConfidenceBadge confidence={analysis.gameWinner.confidence} />
        </div>
        <div className="flex items-center justify-center gap-8">
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500">Seahawks</p>
          </div>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500">Patriots</p>
          </div>
        </div>
        <p className="text-xs text-slate-400 text-center mt-4">{analysis.finalScore.reasoning}</p>
      </div>
      ...
      >
        ...
        <p className="text-sm text-slate-300">{analysis.total.analysis}</p>
            ...
            <div key={i} className="p-4 bg-slate-800/30 rounded-lg flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <span className="text-xs font-mono text-slate-400">{prop.odds}</span>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {analysis.halfProps.map((prop, i) => (
                <div key={i} className="p-3 bg-slate-800/30 rounded-lg">
                  ...
                  <p className="text-xs text-slate-400 mt-1">{prop.reasoning}</p>
                </div>
              ))}
            </div>
          </div>
      ...
      >
        <div className="grid gap-4">
          {analysis.parlays.map((parlay, i) => (
            <div key={i} className="p-4 bg-gradient-to-r from-amber-500/5 to-transparent rounded-xl border border-amber-500/20">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-slate-700/50">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] text-slate-600">per $10 bet</p>
                </div>
              </div>
            </div>
          ))}
      ...
      >
        <div className="grid md:grid-cols-2 gap-4">
          <div className="p-4 bg-gradient-to-br from-amber-500/10 to-transparent rounded-xl border border-amber-500/20">
            <p className="text-xs text-amber-500 uppercase tracking-wider mb-2">Top Pick</p>
            <div className="flex items-center gap-3 mb-2">
...
function Section({ 
  ...
  return (
    <div className="bg-slate-900/30 rounded-xl border border-slate-800/30 overflow-hidden">
      <button
        onClick={onToggle}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {badge && (
            <span className="px-2 py-0.5 bg-slate-700/50 rounded text-xs text-slate-400">{badge}</span>
          )}
        </div>
        <svg 
